...
//打印机将会停止工作，直到被重置。
#define BED_Heating_Waiting_Time 120                             //秒
#define BED_Temperature_Rise_During_Heating_Wait_Time 2          //摄氏度

//电源控制(true/false)
//用NEOPIXEL灯带PIN代替电源控制电源PIN
//启用电源控制系统
#define PSU_CONTROL_Key true
//如果机器闲置此时间，自动关闭电源
//闲置5分钟后自动关机
#define POWER_TIMEOUT_Key 5*60  //秒

//热床探测点个数
//X轴探测3个点
#define GRID_MAX_POINTS_X_KEY 3
//Y轴个数与X轴相同
...
//热床探测点个数
//X轴探测5个点
#define GRID_MAX_POINTS_X_KEY 5
//Y轴个数与X轴相同
#define GRID_MAX_POINTS_Y_KEY GRID_MAX_POINTS_X_KEY
...
//   if (fmod(Number_Of_Probes_Per_Point, 1) != 0) { 
//     #error "Number_Of_Probes_Per_Point必须是整数"
//   }

//   return 0;
// }
// //The_Number_Of_Data_That_Is_Allowed_To_Be_Discarded
// int main() {

//   if (fmod(The_Number_Of_Data_That_Is_Allowed_To_Be_Discarded, 1) != 0) { 
//     #error "The_Number_Of_Data_That_Is_Allowed_To_Be_Discarded"
//   }
...
//确定断料传感器启用状态正确
#if defined(FILAMENT_RUNOUT_SENSOR_Key_Switch)
    #if FILAMENT_RUNOUT_SENSOR_Key_Switch != true && FILAMENT_RUNOUT_SENSOR_Key_Switch != false
    #error "FILAMENT_RUNOUT_SENSOR_Key_Switch 必须是布尔值。"
    #endif
#endif
...
#elif !defined(DEFAULT_AXIS_STEPS_PER_UNIT_X) && !defined(DEFAULT_AXIS_STEPS_PER_UNIT_Y) && !defined(DEFAULT_AXIS_STEPS_PER_UNIT_Z) && !dfined(DEFAULT_AXIS_STEPS_PER_UNIT_E)
#else 
#error "\"DEFAULT_AXIS_STEPS_PER_UNIT_X\"、\"DEFAULT_AXIS_STEPS_PER_UNIT_Y\" 、\"DEFAULT_AXIS_STEPS_PER_UNIT_Z\"和 \"DEFAULT_AXIS_STEPS_PER_UNIT_E\" 都需要同时被定义或者同时不被定义"
#endif
...
#elif !defined(DEFAULT_MAX_FEEDRATE_X) && !defined(DEFAULT_MAX_FEEDRATE_Y) && !defined(DEFAULT_MAX_FEEDRATE_Z) && !dfined(DEFAULT_MAX_FEEDRATE_E)
#else 
#error "\"DEFAULT_MAX_FEEDRATE_X\"、\"DEFAULT_MAX_FEEDRATE_Y\" 、\"DEFAULT_MAX_FEEDRATE_Z\"和 \"DEFAULT_MAX_FEEDRATE_E\" 都需要同时被定义或者同时不被定义"
#endif
...
#elif !defined(DEFAULT_MAX_ACCELERATION_X) && !defined(DEFAULT_MAX_ACCELERATION_Y) && !defined(DEFAULT_MAX_ACCELERATION_Z) && !dfined(DEFAULT_MAX_ACCELERATION_E)
#else 
#error "\"DEFAULT_MAX_ACCELERATION_X\"、\"DEFAULT_MAX_ACCELERATION_Y\" 、\"DEFAULT_MAX_ACCELERATION_Z\"和 \"DEFAULT_MAX_ACCELERATION_E\" 都需要同时被定义或者同时不被定义"
#endif
...
#elif !defined(DEFAULT_ACCELERATION_Key) && !defined(DEFAULT_RETRACT_ACCELERATION_Key) && !defined(DEFAULT_TRAVEL_ACCELERATION_Key) && !dfined(DEFAULT_TRAVEL_ACCELERATION_Key)
#else 
...
#elif !defined(NOZZLE_TO_PROBE_OFFSET_X) && !defined(NOZZLE_TO_PROBE_OFFSET_Y) && !defined(NOZZLE_TO_PROBE_OFFSET_Z) && !dfined(DEFAULT_TRAVEL_ACCELERATION_Key)
#else 
...
#elif !defined(INVERT_X_DIR_Key) && !defined(INVERT_Y_DIR_Key) && !defined(INVERT_Z_DIR_Key) && !dfined(INVERT_E_DIR_Key)
#else 
#error "\"INVERT_X_DIR_Key\"、\"INVERT_Y_DIR_Key\" 、\"INVERT_Z_DIR_Key\"和 \"INVERT_E_DIR_Key\" 都需要同时被定义或者同时不被定义"
#endif

//确保打印区域同时被定义
#if defined(Printable_Area_X) && defined(Printable_Area_Y) && defined(Printable_Area_Z) && defined(DEFAULT_TRAVEL_ACCELERATION_Key)
#elif !defined(Printable_Area_X) && !defined(Printable_Area_Y) && !defined(Printable_Area_Z) && !dfined(DEFAULT_TRAVEL_ACCELERATION_Key)
#else 
...
#elif !defined(X_DRIVER_TYPE_KEY) && !defined(Y_DRIVER_TYPE_KEY) && !defined(Z_DRIVER_TYPE_KEY) && !dfined(E_DRIVER_TYPE_KEY)
#else 
#error "\"X_DRIVER_TYPE_KEY\"、\"Y_DRIVER_TYPE_KEY\" 、\"Z_DRIVER_TYPE_KEY\"和 \"E_DRIVER_TYPE_KEY\" 都需要同时被定义或者同时不被定义"
#endif
...
#error "\"GRID_MAX_POINTS_X_KEY\"和 \"GRID_MAX_POINTS_Y_KEY\" 都需要同时被定义或者同时不被定义"
#endif
...
#elif !defined(Z_CLEARANCE_DEPLOY_PROBE_KEY) && !defined(Z_CLEARANCE_BETWEEN_PROBES_KEY) && !defined(Z_CLEARANCE_MULTI_PROBE_KEY) && !dfined(DEFAULT_TRAVEL_ACCELERATION_Key)
#else 
