...
//采用key_configuration_parameters.h的定义优先覆盖
  ...
  #define E0_DRIVER_TYPE E_DRIVER_TYPE_KEY
#elif !defined(X_DRIVER_TYPE_KEY) && !defined(Y_DRIVER_TYPE_KEY) && !defined(Z_DRIVER_TYPE_KEY) && !defined(E_DRIVER_TYPE_KEY)
  // 什么也不做
#else
  #error "请检查驱动类型是否全部定义。X_DRIVER_TYPE_KEY, Y_DRIVER_TYPE_KEY, Z_DRIVER_TYPE_KEY, E_DRIVER_TYPE_KEY 都需要同时被定义或者同时不被定义。"
#endif
...
// 如果定义了 PSU_CONTROL_Key，则使用 key_configuration_parameters.h 中的定义来覆盖 PSU_CONTROL 的定义
  ...
  #if PSU_CONTROL_Key
    // 如果 PSU_CONTROL_Key 为真，则检查是否定义了 PSU_CONTROL
    #if !defined(PSU_CONTROL)
        // 如果没有定义，则定义它
        #define PSU_CONTROL
    #endif
  #else
    // 如果 PSU_CONTROL_Key 为假，则检查是否定义了 PSU_CONTROL
      #if defined(PSU_CONTROL)
        // 如果定义了，则取消定义它
        #undef PSU_CONTROL
      #endif
...
//采用key_configuration_parameters.h的定义优先覆盖
  ...
  #endif
    #if defined(Z_CLEARANCE_BETWEEN_PROBES)
    #undef Z_CLEARANCE_BETWEEN_PROBES
  #endif
    #if defined(Z_CLEARANCE_MULTI_PROBE)
    #undef Z_CLEARANCE_MULTI_PROBE
  #endif
  #define Z_CLEARANCE_DEPLOY_PROBE Z_CLEARANCE_DEPLOY_PROBE_KEY
  #define Z_CLEARANCE_BETWEEN_PROBES Z_CLEARANCE_BETWEEN_PROBES_KEY
  #define Z_CLEARANCE_MULTI_PROBE Z_CLEARANCE_MULTI_PROBE_KEY
#endif
...
#undef INVERT_E0_DIR
#define INVERT_E0_DIR INVERT_E_DIR_Key
#endif
...
//采用key_configuration_parameters.h的定义优先覆盖
  ...
  #endif
  #if defined(GRID_MAX_POINTS_Y)
    #undef GRID_MAX_POINTS_Y
  #endif
  #define GRID_MAX_POINTS_X GRID_MAX_POINTS_X_KEY
