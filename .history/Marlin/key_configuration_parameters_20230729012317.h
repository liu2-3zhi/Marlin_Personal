...
// 检查打印区域值是否大于70
#if Printable_Area_X < 70
    #error "Printable_Area_X 必须大于70mm"  
#endif

#if Printable_Area_Y < 70
...
//确保热保护温度大于等于2
#if E_Temperature_Rise_During_Heating_Wait_Time <  2
    #error "E_Temperature_Rise_During_Heating_Wait_Time 必须大于等于2"
#endif
#if BED_Temperature_Rise_During_Heating_Wait_Time <  2
...
//确定X轴探测个数的正确
    ...
    #error "GRID_MAX_POINTS_X_KEY需要大于0"
#endif

//确定Y轴探测个数的正确
#if GRID_MAX_POINTS_Y_KEY <= 0
    #error "GRID_MAX_POINTS_Y_KEY需要大于0"
#endif

#define DEFAULT_Kp_Key 120
static_assert(DEFAULT_Kp_Key > 0.0f, "Value must be positive");
//————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————