...
// 检查默认PID参数是否大于0
//直接转换DEFAULT_Kp_Key为整数进行比较
#if (int)(DEFAULT_Kp_Key) <= 0

#error "DEFAULT_Kp_Key必须大于0"
#endif

#if (int)(DEFAULT_Ki_Key) <= 0
#error "DEFAULT_Ki_Key必须大于0"
#endif

#if (int)(DEFAULT_Kd_Key) <= 0
#error "DEFAULT_Kd_Key必须大于0"
#endif

// 检查热床PID参数是否大于0
//使用中间变量pid_value保存为浮点数
//然后转换为整数进行比较
#if float pid_value = DEFAULT_bedKp_Key; (int)(pid_value) <= 0

#error "DEFAULT_bedKp_Key必须大于0"
#endif

#if float pid_value = DEFAULT_bedKi_Key; (int)(pid_value) <= 0
#error "DEFAULT_bedKi_Key必须大于0"

#endif

#if float pid_value = DEFAULT_bedKd_Key; (int)(pid_value) <= 0
#error "DEFAULT_bedKd_Key必须大于0"
#endif

//Number_Of_Probes_Per_Point是整数类型
//可以直接比较是否为整数
